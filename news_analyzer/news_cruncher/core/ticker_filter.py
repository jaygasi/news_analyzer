...
class TickerFilterEngine:
    ...
    def filter_ticker_buckets(self, ticker_buckets: Dict[str, List[Dict[str, Any]]]) -> Dict[str, List[Dict[str, Any]]]:
        ...
        log_info("🔑 API Configuration Check:")
    ...
    def _get_batch_fundamentals(self, tickers: List[str]) -> Dict[str, Dict[str, Any]]:
        """OPTIMIZED: Get fundamentals using FMP bulk API calls - MAJOR PERFORMANCE IMPROVEMENT"""
        if not tickers:
            return {}
        log_info("🔍 BULK API DIAGNOSTICS:")
        log_info(f"   Input tickers: {len(tickers)}")
        log_info(f"   API Key present: {'✅' if self.fmp_loader.api_key else '❌'}")
        log_info(f"   Cache enabled: {'✅' if hasattr(self, 'fundamentals_cache') else '❌'}")
        ...
        if not tickers_to_fetch:
            log_debug(f"📋 All {len(tickers)} tickers served from cache")
            return results

        log_info(f"🚀 BULK API processing {len(tickers_to_fetch)} tickers (cached: {len(results)})")

        # CRITICAL OPTIMIZATION: Process in bulk chunks
        # FMP API supports bulk requests: /profile/AAPL,MSFT,GOOGL (up to 20 tickers)
        chunk_size = 20  # FMP bulk API limit
        total_chunks = (len(tickers_to_fetch) + chunk_size - 1) // chunk_size

        for chunk_num in range(total_chunks):
            start_idx = chunk_num * chunk_size
            end_idx = min(start_idx + chunk_size, len(tickers_to_fetch))
            chunk_tickers = tickers_to_fetch[start_idx:end_idx]

            log_info(f"📡 Bulk API call {chunk_num + 1}/{total_chunks} for {len(chunk_tickers)} tickers")

            try:
                # OPTIMIZATION: Single bulk API call instead of individual calls
                ticker_string = ','.join(chunk_tickers)

                # Apply rate limiting once per bulk request (not per ticker)
                self._rate_limit()

                # Make bulk profile request
                profile_data = self.fmp_loader.make_request(f"profile/{ticker_string}")

                # Improved validation and error handling
                if profile_data and isinstance(profile_data, list) and len(profile_data) > 0:
                    # Process bulk profile response
                    profile_by_symbol = {}
                    for item in profile_data:
                        if isinstance(item, dict) and 'symbol' in item:
                            symbol = item.get('symbol', '').strip().upper()
                            if symbol and symbol in chunk_tickers:
                                profile_by_symbol[symbol] = item
                                log_debug(f"📊 Profile data received for {symbol}")
                        else:
                            log_warning(f"⚠️ Invalid profile item in bulk response: {item}")

                    log_info(f"📡 Bulk profile call returned {len(profile_by_symbol)}/{len(chunk_tickers)} valid profiles")

                    # Get bulk quote data for the same tickers
                    self._rate_limit()
                    quote_data = self.fmp_loader.make_request(f"quote/{ticker_string}")

                    quote_by_symbol = {}
                    if quote_data and isinstance(quote_data, list):
                        for item in quote_data:
                            symbol = item.get('symbol', '')
                            if symbol in chunk_tickers:
                                quote_by_symbol[symbol] = item

                    # Compile fundamentals for each ticker in the chunk
                    for ticker in chunk_tickers:
                        profile = profile_by_symbol.get(ticker)
                        quote = quote_by_symbol.get(ticker)

                        if profile and quote:
                            # Check options availability (simplified check for bulk processing)
                            has_options = self._check_options_availability_bulk(ticker)

                            # Compile fundamental data
                            fundamentals = {
                                'price': float(profile.get('price', 0) or (quote.get('price', 0) if quote else 0)),
                            ...
                            if fundamentals['price'] > 0 and fundamentals['market_cap'] > 0:
                                results[ticker] = fundamentals
                                self._cache_fundamentals(ticker, fundamentals)
                                log_debug(f"✅ {ticker}: Bulk data compiled - Price: ${fundamentals['price']:.2f}")
                            else:
                                log_warning(f"❌ {ticker}: Invalid bulk data - Price: {fundamentals['price']}, Cap: {fundamentals['market_cap']}")
                        else:
                            log_warning(f"❌ {ticker}: Missing profile or quote data in bulk response")

                else:
                    log_warning(f"❌ Bulk profile request failed for chunk {chunk_num + 1}")
                    # Fallback to individual processing for this chunk only
                    for ticker in chunk_tickers:
                        try:
                            self._rate_limit()
                            individual_fundamentals = self._get_company_fundamentals(ticker)
                            if individual_fundamentals:
                                results[ticker] = individual_fundamentals
                        except Exception as e:
                            log_warning(f"❌ Individual fallback failed for {ticker}: {e}")

            except Exception as e:
                log_error(f"❌ Bulk processing failed for chunk {chunk_num + 1}: {e}")
            ...
            # Brief delay between bulk chunks (much shorter than individual requests)
            if chunk_num + 1 < total_chunks:
                time.sleep(0.2)

        # Reset consecutive errors on successful bulk processing
        self._consecutive_errors = 0

        log_info(f"⚡ Bulk processing complete: {len(results)}/{len(tickers)} fundamentals retrieved")
        return results
    ...
    def _get_company_fundamentals(self, ticker: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            profile_data = self.fmp_loader.make_request(f"profile?symbol={ticker}")
            ...
            quote_data = self.fmp_loader.make_request(f"quote?symbol={ticker}")
    ...
    def clear_failed_ticker_cache(self) -> None:
        ...
        try:
            if hasattr(self.fmp_loader, '_clear_failed_ticker_cache'):
                ...
                log_warning("⚠️ Failed ticker cache clearing not available in FMP loader")
        except Exception as e:
            log_error(f"❌ Error clearing failed ticker cache: {e}")

    def _validate_ticker_exists(self, ticker: str) -> bool:
        """Validate ticker exists with comprehensive caching"""

        # Check existing failed ticker cache first
        if self.fmp_loader._is_ticker_cached_as_failed(ticker):
            log_debug(f"⏭️ Skipping cached failed ticker (validation): {ticker}")
            return False

        try:
            # Strategy 1: Quick quote validation
            ...
            if quote_data and isinstance(quote_data, list) and quote_data:
                ...
                # A very small market cap might indicate a delisted or problematic ticker
                if price and price > 0 and (market_cap is None or market_cap > 10000): # Check for valid price and reasonable market cap
                    log_debug(f"✅ {ticker} validated via quote: Price ${price:.2f}, Cap {market_cap}")
                    return True

            # Strategy 2: Company profile validation
            # Apply rate limit before API call
            self._rate_limit()
            ...
            if profile_data and isinstance(profile_data, list) and profile_data:
                ...
                # Ensure company name exists and price is valid
                if company_name and price and price > 0:
                    log_debug(f"✅ {ticker} validated via profile: {company_name}, Price ${price:.2f}")
                    return True

            # All strategies failed - cache as invalid
            log_warning(f"❌ {ticker} failed validation (no valid price/profile) - caching as invalid")
            self.fmp_loader._add_failed_ticker_to_cache(ticker, "ticker_validation_failed")
            return False

        except Exception as e:
            log_error(f"❌ Error validating ticker {ticker}: {e}")
            self.fmp_loader._add_failed_ticker_to_cache(ticker, f"validation_error_{type(e).__name__}")
            return False

    def pre_validate_tickers(self, ticker_buckets: Dict[str, List[Dict[str, Any]]]) -> Dict[str, List[Dict[str, Any]]]:
        """Pre-validate tickers to remove invalid ones before expensive analysis"""
        if not getattr(Config, 'ENABLE_TICKER_PRE_VALIDATION', True): # Check config flag
            log_info("📋 Ticker pre-validation disabled by Config.ENABLE_TICKER_PRE_VALIDATION.")
            return ticker_buckets

        log_info(f"🔍 Pre-validating {len(ticker_buckets)} tickers before fundamental analysis...")

        valid_buckets = {}
        invalid_tickers_count = 0

        for ticker, articles in ticker_buckets.items():
            if self._validate_ticker_exists(ticker):
                valid_buckets[ticker] = articles
            else:
                invalid_tickers_count += 1
                # Detailed logging for failed validation is handled within _validate_ticker_exists

        log_info(f"📊 Pre-validation complete: {len(valid_buckets)} valid tickers, {invalid_tickers_count} invalid/skipped.")
