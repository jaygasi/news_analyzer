...
# Pylance: The following imports require their respective libraries to be installed (e.g., pip install torch transformers google-generativeai openai anthropic).
import torch
...
from analysis.emergency_data_services import EmergencyDataServices, DirectionalPrediction as EDSDirectionalPrediction # Alias for type clarity
...
class MultiLLMAnalyzer:
    ...
    def analyze_news_direction(self, ticker: str, articles: List[Dict[str, Any]]) -> Optional[DirectionalPrediction]:
        ...
        Tries LLMs first, then emergency fallbacks, then keyword analysis.
        """
        if not articles:
            return None
        ...
        llm_prediction = self._try_analysis_chain(Config.LLM_PRIORITY, ticker, combined_text, self._analyze_with_service)
        if llm_prediction:
            return llm_prediction

        # Try real emergency fallbacks if all LLMs exhausted
        log_warning("All LLMs exhausted, trying emergency data services")
        emergency_prediction = self._try_analysis_chain(Config.EMERGENCY_FALLBACKS, ticker, combined_text, self._analyze_with_emergency_service)
        ...
        return self._enhanced_keyword_analysis(combined_text) # Removed unused 'ticker' parameter

    def _try_analysis_chain(self, service_names: List[str], ticker: str, text: str,
                            analysis_method: Any) -> Optional[DirectionalPrediction]:
        """Helper to iterate through a list of services and return the first successful prediction."""
        for service_name in service_names:
            if self._is_service_available(service_name):
                try:
                    prediction = analysis_method(service_name, ticker, text)
                    if prediction:
    ...
    def _analyze_with_emergency_service(self, service_name: str, ticker: str, text: str) -> Optional[DirectionalPrediction]:
        ...
        try:
            eds_prediction: Optional[EDSDirectionalPrediction] = None # Explicitly type it for clarity
    ...
    def _analyze_with_gemini(self, ticker: str, text: str) -> Optional[DirectionalPrediction]:
        ...
        try:
            ...
            if response and response.text:
                return self._parse_llm_response(response.text, "gemini") # Removed unused 'ticker' parameter
            return None
    ...
    def _analyze_with_openai(self, ticker: str, text: str) -> Optional[DirectionalPrediction]:
        ...
        try:
            ...
            if response.choices and response.choices[0].message.content:
                return self._parse_llm_response(response.choices[0].message.content, "openai") # Removed unused 'ticker' parameter
            return None
    ...
    def _analyze_with_claude(self, ticker: str, text: str) -> Optional[DirectionalPrediction]:
        ...
        try:
            ...
            if response.content and response.content[0].text:
                return self._parse_llm_response(response.content[0].text, "claude") # Removed unused 'ticker' parameter
            return None
...
"""
    ...
    def _parse_llm_response(self, response_text: str, source: str) -> Optional[DirectionalPrediction]: # Removed unused 'ticker' parameter
    ...
    def _enhanced_keyword_analysis(self, text: str) -> Optional[DirectionalPrediction]: # Removed unused 'ticker' parameter
